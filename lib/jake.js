...
function run(dirname, ns, validTasks, args)
{
  ...
  {
    return (typeof arg === 'string' && arg.substr(0, 1) === '-') ? false : true;
  }

  // keep support of old style gen syntax
  if (args.length > 0 && validTasks.indexOf(args[0]) === -1 && args[0].search(/[a-z]\[.*\]/) === -1) {
  ...
  if (args.length && args[0].indexOf('default') !== 0) {
